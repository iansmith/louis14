...
4. **Cmt-4 (re-apply Cmt-B) — NEXT.** Post-loop break guard from `docs/PLAN-phase-22.md` §14.2. Continuation prompt: `docs/PROMPT-phase-25-cmt-4.md`. With Cmt-3's OOF pipeline in place, Cmt-B's break-token emission is no longer broken by the OOF path — the inner multicol can fragment across outer columns and the OOFs ride the new fragmentainers via the drain. Closure target: `-011` and `-032` at 0%.
...
**Worktree.** `phase-25-oof-fragmentation` from master. Currently at `7ed644d7` (Cmt-3b drain pipeline, gate-neutral; full multicol suite 212/243 unchanged from Cmt-2 baseline). Cmt-4 next.
