...
5. **Cmt-5 (Phase 14b reshape) — NEXT.** Continuation prompt: `docs/PROMPT-phase-25-cmt-5.md`. Two sub-problems: (a) eliminate the 200 px `-033` regression that surfaces when Phase 14b is narrowed (likely an OOF-positioning gap in Cmt-3's drain or in the cross-fragmentation interaction); (b) once (a) holds, narrow Phase 14b. Residual ~1.0% on `-011`/`-032` after Cmt-5 likely needs Cmt-6 chasing the `contain:size` content-fragmentation-through-resume bug per `PLAN-phase-22.md` §14.8 (column-content `BlockSizeForFragmentation` + inner row-stride misalignment on resume).
