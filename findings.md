...
### Phase 13d landing notes (2026-04-25)

13d migrated every plan-named ConstraintSpace and ExclusionSpace precision-edge field off `float64` and onto LayoutUnit. Five checkpoint commits, each gate-swept; all six invariants held at every step. The bridge-helper pattern from 13c carried over directly: setters keep their float64 signatures and convert internally; readers add `.Float64()` at the access site; whole-Size value passes wrap through `geomLogicalToOld`/`oldLogicalToGeom`.

**Sub-step ordering (smallest blast radius first).** Same discipline as 13c: ConsumedBlockSize (~24 sites) → ClearanceOffset (~13 sites, but a method signature change) → Bfc offsets (~30 sites) → AvailableSize (~140 sites) → PercentageResolutionSize (~23 sites). The big AvailableSize commit (13d.4a) was the only step that materially stressed the bridge-helper pattern — it surfaced every site where the old converter API meets a constraint-space-typed value.

**Setters convert internally; callers don't change.** This was the load-bearing simplification. `SetAvailableSize(LogicalSize)` still takes the old float64 type; the 80+ call sites that build LogicalSize literals don't move. Inside the setter we wrap via `oldLogicalToGeom`. The asymmetry (set with old type, read with new type) is acceptable during migration because the alternative — changing setter signatures — would have multiplied the blast radius by 4 with zero precision benefit (callers already have float64 in hand from CSS resolution; converting at setter boundary is the same point).

**Indefinite sentinel round-trips cleanly.** `pkg/layout.Indefinite = float64(-1)` and `layoutunit.IndefiniteSize` (raw=-64, value=-1px) are by-construction equal under `FromFloat64Round(-1).Float64()`. So consumer code like `if x.Float64() == Indefinite { ... }` continues to work without conditional logic. This was a nice surprise of the LayoutUnit design — Blink picked the same sentinel value for the same reason.

**Bridge helpers shrink, they don't grow.** The 4 helpers in `physical.go` (geomSizeToOld/oldSizeToGeom from 13c, geomLogicalToOld/oldLogicalToGeom from 13d) are transitional. They're each one line, with a comment naming them "during migration." When 13e migrates length resolution and the legacy float64 converter API moves to LayoutUnit, these helpers go away mechanically. The shape of the migration is: drop a bridge in commit N, migrate consumers in N+1..N+k, delete the bridge in N+k+1.

**Mutations are simpler at field-level than at value-level.** `frag.Size.Height = layoutunit.FromFloat64Round(spanHeight)` works because `geometry.PhysicalSize` exports its LayoutUnit fields. We don't need a setter method on the geometry type. The 13c precedent for this (multicol spanner clip) carried over to the 13d.1 ConsumedBlockSize accumulator pattern (`tok.ConsumedBlockSize = tok.ConsumedBlockSize.Add(...)`) and to the BfcContainerInlineSize "is set" check (`!.IsZero()` instead of `> 0`).
...
### Phase 13g.2 verification (2026-04-26)

Side-by-side instrumentation — gated by `LOUIS14_SNAP_DEBUG=1`, removed before commit — printed both the OLD (`math.Round(snapped_outer + bw)`) and NEW (`outerEdge ± SnapSizeToPixel(bw, unsnapped_origin)`) inner-edge values for every painted box during a throwaway test (`pkg/visualtest/debug_phase13g_test.go`, also removed before commit) that rendered seven WPT border tests. Two findings were load-bearing for the migration's correctness:

**Finding 1: Thin-line preservation works.** With `border-width: 0.1px` and `0.25px`, the OLD code produced thickness `L0 T0 R0 B0` — borders vanished entirely (`math.Round(0.1) = 0`). The NEW code produces `L1 T1 R1 B1` because the `>4 raw / <-4 raw` clause in `SnapSizeToPixel` forces ±1px when the snap collapses to 0 but the user asked for a real (if thin) line. The WPT tests `border-width-small-values-001-{a,b,c}.html` (which use `rel="mismatch"` and are skipped by the runner) are the canonical reproduction.

**Finding 2: Asymmetric 0.5px border bug surfaced.** With a uniform `border: 0.5px solid black`, the OLD code produced thickness `L1 T1 R0 B0` — left and top got 1px, but right and bottom got 0px. Mechanism: with snapped `outerLeft = 8`, `outerRight = 792`, and `bw.Right = 0.5`, `Round(8 + 784 - 0.5) = Round(791.5) = 792` (round-half-away-from-zero on a positive value). So `innerRight - outerRight = 0`, collapsing the right border. The NEW code computes `outerRight - SnapSizeToPixel(0.5, 791.5)` — fraction(791.5) = 0.5; `(0.5 + 0.5).Round() - Round(0.5) = 1 - 1 = 0`; thin-line clause fires (size 32 raw > 4 raw); returns 1. Symmetric 1px on all four sides. **Latent bug, surfaced by the migration; would have caused user-visible asymmetry on any test with a 0.5px symmetric border.**

**Finding 3: Parallel-sites scope correction.** The original 13g research listed four sites that all match `pixelSnap(box.X, box.Y, box.Width, box.Height)`: `:2669` `drawBorders`, `:2779` `drawRoundedBorders`, `:2940` `drawOutline`, `:4209` `drawOutsetBoxShadows`. Reading the code confirmed only `:2669` has the inner-corner `math.Round(x + bw.Left)` shape — the other three use different geometry (radius midline, outline expand-outward, shadow rect). **Only :2669 is a 13g.2 target.** The original "(and the parallel sites at :2779, :2940, :4209)" annotation in the staging plan was over-eager.
