...
## Current state (2026-05-01 — Cmt-5a + 5b + 5e landed; -011 closure blocked by Phase 21 hard-blocker)
...
| css-multicol (master) | 212/455 | Phase 22 Cmt-1 (`75182bd6`) closed `broken-column-rule-1`. Phase 25 closure landed 2026-05-01 on `phase-25-cmt-5ab-retry` at 215/455 (Cmt-5a + 5b + 5e; `-032` closed). Not yet merged to master. `-011` cluster visual closure still blocked by Phase 21 hard-blocker (8/9 prior-clip-wins regress without underlying layout overflow fixes). |
| css-multicol (phase-25-cmt-5ab-retry) | 215/455 | Cmt-5a (`1ca0904d`) + real Cmt-5b (`e10ff3a2`) + Cmt-5e (`d01a3a36`) on top of Cmt-5d. Drivers **15/15** (was 14/15; `-032` closes via Cmt-5e). Prior-clip-wins 9/9. Spanner-fragmentation 12/13 (`-008` pre-existing). Targeted: `-010`/`-032`/`-033` PASS, `-011` 5000 px (Phase 21 clip-blocked). |
...
### Phase 25 — Fragmentation-aware OOF positioning (Blink-aligned port) — Cmt-5a + 5b + 5e ALL LANDED 2026-05-01 (gate 212→215; -011 visual closure now blocked on Phase 21)
...
5. **Cmt-5 (four-step closure plan) — IN PROGRESS.** Continuation prompt: `docs/PROMPT-phase-25-cmt-5.md` (rewritten 2026-04-30 from a four-agent Blink + local-code investigation). The original "narrow Phase 14b" Cmt-5 plan was wrong — narrowing alone leaves four distinct upstream bugs unfixed. Sequence:
   ...
   - **Cmt-5e (Phase 14b narrowing) — DONE 2026-05-01 (`d01a3a36` on `phase-25-cmt-5ab-retry`).** Narrowed Phase 14b's deferral (`multicol_layout.go:423`) to fire only when `mla.space.FragmentainerOffset > 0`. Inner multicols starting fresh at outer offset 0 now fragment in place rather than being deferred (which would loop indefinitely when the next outer column also has insufficient space). Closure: drivers 14/15 → 15/15 (`-032` closes); `-033` stays 0/0; `-010` stays 0/0. Full multicol gate 214 → 215/455. **`-011` does NOT close** (stays 5000 px) — its bottom-half-green coverage requires the contain-size's overflow to extend past outer-mc's box, which is currently truncated by the unconditional P20.5 multicol clip. That is Phase 21 territory.

**Phase 21 hard-blocker (2026-05-01 investigation).** Probed Phase 21 (gate the P20.5 unconditional multicol clip on user-set `overflow != visible`) on top of 5a+5b+5e: **8 of 9 prior-clip-wins regress** (multicol-breaking-002 7.8%, fill-balance-nested-000 0.3%, list-item-001 0.3%, nested-015 0.3%, nested-021 1.3%, nested-026 1.0%, nested-028 2.2%, nested-after-float-clearance 1.6%; only nobackground-002 holds). Phase 25's chain semantics did NOT eliminate all the layout-overflow shapes the clip is masking — the 9 prior-clip-wins use a wide variety of overflow patterns (text wrap, float clearance, list markers, nested balancing) that need separate per-test root-causing.

Additional finding for `-011`: even with Phase 21 + Cmt-5a reverted (i.e. `contain:size` IsMonolithic restored), `-011` does not close — louis14's painter clamps a monolithic box's painted block-size to the fragmentainer's slice size, so `contain:size` overflow does not extend past the fragmentainer's block boundary in the painter. To close `-011` to 0/0 would require: (1) Phase 21 PLUS underlying overflow fixes for ≥ 8 prior-clip-wins, AND (2) a paint-side fix for monolithic block-direction overflow past fragmentainer boundaries. Neither is in scope for Phase 25.

**Status (2026-05-01).** Cmt-5a (`1ca0904d`) + Cmt-5b (`e10ff3a2`) + Cmt-5e (`d01a3a36`) all landed on `phase-25-cmt-5ab-retry`. Gate 212→215/455 (`-032` closed via Cmt-5e). Drivers 15/15, prior-clip-wins 9/9, spanner-frag 12/13, 4-cat invariants intact. Not yet merged to master. Worktree `/Users/iansmith/louis14-p25-cmt5ab` HEAD at `d01a3a36`. **Phase 25 work for `-011` is functionally complete** — chain semantics correct, fragmentation flows properly. **`-011` cluster visual closure NOT achieved**: requires Phase 21 + at least 8 prior-clip-win underlying-overflow fixes + a paint-side fix for monolithic block-direction overflow past fragmentainers. That is a separate phase.
...
**Worktree.** `phase-25-cmt-5ab-retry` from `e8f25761` (Cmt-5d). At `/Users/iansmith/louis14-p25-cmt5ab`. Currently at `d01a3a36` (Cmt-5e on top of Cmt-5b on top of revert-of-agent's-Cmt-5b on top of Cmt-5a on top of Cmt-5d). The reverted agent attempt `57bb8967` is preserved in git history for reference.
