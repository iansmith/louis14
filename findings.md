...
## Current state (2026-05-01 — Cmt-5a landed, Cmt-5b REVERTED — actual chain bug still open)
...
| css-multicol (master) | 212/455 | Phase 22 Cmt-1 (`75182bd6`) closed `broken-column-rule-1`. Phase 25 PARKED 2026-05-01 at Cmt-5d (`e8f25761` on `phase-25-oof-fragmentation`); gate-neutral. Cmt-5a/b/e deferred — see § Phase 25. Cmt-5a landed 2026-05-01 on `phase-25-cmt-5ab-retry` (gate-neutral; not yet merged to master). |
| css-multicol (phase-25-cmt-5ab-retry) | 214/455 | Cmt-5a (`1ca0904d`) only on top of Cmt-5d. Cmt-5b reverted 2026-05-01 (`dcd6f40d`) — agent's fix didn't address the chain bug and regressed `-032` under Cmt-A. Drivers 14/15 (`-032` pre-existing). Prior-clip-wins 9/9. Real Cmt-5b retry pending — see `docs/PROMPT-phase-25-cmt-5b-continuation.md`. |
...
### Phase 25 — Fragmentation-aware OOF positioning (Blink-aligned port) — Cmt-5a landed; Cmt-5b REVERTED; Cmt-5e DEFERRED
...
5. **Cmt-5 (four-step closure plan) — IN PROGRESS.** Continuation prompt: `docs/PROMPT-phase-25-cmt-5.md` (rewritten 2026-04-30 from a four-agent Blink + local-code investigation). The original "narrow Phase 14b" Cmt-5 plan was wrong — narrowing alone leaves four distinct upstream bugs unfixed. Sequence:
   ...
   - **Cmt-5b (BlockBreakToken child-chaining) — REVERTED 2026-05-01 (`dcd6f40d` reverts `57bb8967` on `phase-25-cmt-5ab-retry`).** Sonnet sub-agent's Cmt-5b set `HasSeenAllChildren=true` when `outBuilder.IsEmpty()` in `buildOuterBreakResult`. Reasonable-looking fix but: (a) the agent skipped the prompt's mandatory Cmt-A verification — under Cmt-A, `-011` still failed at 5000px / 1.0% (the exact pre-fix symptom); (b) under Cmt-A, the fix actively REGRESSED `-032` from PASS to 5000px / 1.0%. So when Cmt-5e (Cmt-A permanent) eventually lands, this commit would have turned `-032` from pass→fail. Bisect: `5a alone + Cmt-A` → `-032` passes; `5a + 5b + Cmt-A` → `-032` fails. The agent's diagnosis (per-loop guard with empty outBuilder re-running content) is RELATED to but NOT the chain bug the prompt described. Real chain bug remains open — see `docs/PROMPT-phase-25-cmt-5b-continuation.md` for full investigation status, trace evidence, and continuation plan.
...
**Status (2026-05-01).** Cmt-5a landed (`1ca0904d`); Cmt-5b reverted (`dcd6f40d`) — agent's fix didn't close `-011` under Cmt-A and regressed `-032` under Cmt-A. Gate 212→214/455 from Cmt-5a alone. Not yet merged to master. Cmt-5b retry pending; Cmt-5e remains deferred. Worktree `/Users/iansmith/louis14-p25-cmt5ab` is still live with HEAD at `dcd6f40d`. Continuation prompt: `docs/PROMPT-phase-25-cmt-5b-continuation.md`.
...
**Worktree.** `phase-25-cmt-5ab-retry` from `e8f25761` (Cmt-5d). At `/Users/iansmith/louis14-p25-cmt5ab`. Currently at `dcd6f40d` (Revert Cmt-5b on top of Cmt-5a on top of Cmt-5d). The reverted Cmt-5b commit `57bb8967` is preserved in git history for reference.
