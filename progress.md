...
|---|---|---|
| css-writing-modes | 781/781 | 2026-04-21 (post Phase 9 third landing) |
| CSS2 (TestWPTReftests) | 99/99 | 2026-04-21 (post Phase 9 third landing) |
| css-flexbox | 626/629 | 2026-04-21 (post Phase 9 third landing) |
| css-position (target → watch post-Phase-9) | 95/100 runnable | 2026-04-21 (post Phase 9 third landing) |
...
| Question | Answer |
|----------|--------|
| Where am I? | **Pivoting to css-multicol (Phase 12).** css-position at 95/100 runnable — phases 1–9 effectively complete; residuals (`clear-001` Blink subpixel quirk, `position-change` parser bug) deferred. css-multicol baseline (2026-04-21) = 94/458 = 20.5%. Phase 12 research + plan landed 2026-04-21; implementation begins 2026-04-22 at Phase 12a (fragmentation infra). Full Blink-source research + louis14 audit + 8-phase plan (12a–12h) in `findings.md` "css-multicol category". Driver test: `multicol-fill-balance-001.html`. |
| Where am I going? | css-multicol 94 → ~300+ via Phase 12a–12h. Invariants must hold: wm 781/781, CSS2 99/99, flex ≥626, css-position ≥95/100, css-transforms ≥172. |
| What's the goal? | Phase 12a: re-architect `pkg/layout/multicol_layout.go` around NG fragmentation. Add `MinimalSpaceShortage` on `LayoutResult`; thread `BlockBreakToken` between columns; add `BlockFragmentationType` / `IsInitialColumnBalancingPass` / `IsInsideBalancedColumns` flags to `ConstraintSpace`. Mirror Blink's outer-stretch / inner-per-column loop verbatim. Target: ~+30 tests minimum (fill-balance cluster subset). |
