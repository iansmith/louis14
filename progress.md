...
Three tracks executed B→C→A, each committed separately.

**Track B — PropagateBaselineFromChild (commit `af2bbb77`).** Added `firstBaseline` / `hasFirstBaseline` / `useLastBaselineForInlineBaseline` / `lastBaseline` fields to `BoxFragmentBuilder`; corresponding `FirstBaseline` / `HasFirstBaseline` / `UseLastBaselineForInlineBaseline` / `LastBaseline` fields on `LayoutResult`. `propagateBaselineFromChild` method on `MulticolLayoutAlgorithm` implements min-semantics for first baseline (earliest column wins) and max-semantics for last baseline (latest column wins); `SetUseLastBaselineForInlineBaseline` called unconditionally (Blink invariant). Two callsites: per-column loop and spanner commit block. Gate: multicol +1 (188→189; baseline propagation unlocked one test). Reverted to 188 by subsequent Track C changes — net 0 visible.

**Track C — UnpositionedListMarker 4-callsite protocol (commit `b66e7dba`).** New file `pkg/layout/unpositioned_list_marker.go` with `UnpositionedListMarker{MarkerNode}` type and stub methods (all no-ops since `ListMarkerBlockNodeIfListMarker()` returns nil — markers still paint-time only). `BlockBreakToken.HasUnpositionedListMarker` field. `LayoutInputNode` gains `IsListItem()`, `ListMarkerBlockNodeIfListItem()` (→nil), `ListMarkerOccupiesWholeLine()` methods. Four callsites in `MulticolLayoutAlgorithm`: (1) constructor seeds marker from node, (2) end-of-Layout places any unclaimed marker, (3) first-column-of-row attempts placement, (4) spanner commit attempts placement. Protocol is a structural no-op in current engine; provides correct Blink-parity scaffolding for future layout-time marker path.

**Track A — GapGeometry + GapDecorationsPainter (commit `058a5442`).** New file `pkg/layout/gap_geometry.go` with `GapGeometry`, `CrossGap`, `MainGap`, `SpannerMainGapType`, `GapDirection`, `GapContainerType`, `EdgeIntersectionState` types and `IsMainDirection`, `IsMultiColSpanner`, `GetGapCenterOffset` methods — mirrors `gap_geometry.h` / `cross_gap.h` / `main_gap.h`. `css.Style.HasColumnRule()` added. `BoxFragmentBuilder.SetGapGeometry` + `Build()` forward; `PhysicalFragment.GapGeometry`, `Box.GapGeometry`, `PaintLayer.GapGeometry` fields wired through `engine.go` and `paint_layer.go`. Seven private fields on `MulticolLayoutAlgorithm` track cross/main gaps, columns-per-row, first-column-offset, gap size, max-columns. Five helper methods: `addMainGap`, `addCrossGap`, `addNumberOfColumnsForCurrentRow`, `updateCrossGapSegmentStates`, `buildGapGeometry`. Call sites wired: `columnGapSize` set at gap resolution; `firstColumnOffset` + `addCrossGap` + `addNumberOfColumnsForCurrentRow` in `layoutLine()` commit section; `addMainGap(SpannerGapStart)` + `addNumberOfColumnsForCurrentRow(-1)` before spanner `AddChild`, `addMainGap(SpannerGapEnd)` after spanner advance; `buildGapGeometry()` before `builder.Build()`. Renderer updated: `drawColumnRules` uses `GapGeometry.CrossGaps` for rule positions when available, skipping spanner-adjacent gaps via `IsMultiColSpanner`; falls back to ad-hoc loop when nil.

**Gate at Track A HEAD (commit `058a5442`):** CSS2 99/99 · flex 626/629 · pos 92/105 · wm 781/781 · multicol 188/455 · spanner-frag 12/13. All six invariants held.

### Phase 14 planning — DONE 2026-04-26

Blink research and detailed plan written for three fragmentation targets. No code changes; research only.

**Gate at planning HEAD (commit `da58d975`, Phase 13h):** CSS2 99/99 · flex 626/629 · position 92/105 · wm 781/781 · multicol 179/455 · spanner-frag 12/13.

**14a — IFC fragmentation guard + empty-child overflow trigger. DONE (commit `87d06be5`, 2026-04-26).** Three-part fix: (1) `inline_layout.go:963`: change `blockOffset > 0` to `bla.space.FragmentainerOffset+blockOffset > 0`, mirroring Blink's `refuse_break_before = (space_left >= fragmentainer_block_size)`. (2) `block_layout.go` collapseThrough: add `childResult.BreakToken == nil` — a 0-height IFC fragment with a break token was being treated as margin-collapsing-through and discarded via `continue` before the overflow check fired. (3) `block_layout.go` else-if: detect `childHasBreak && childBlockSize == 0 && fragSize != Indefinite && !IsInitialColumnBalancingPass` as overflow when `blockCursor < fragEnd`. Closes all 4 F4 regressions. Gate: CSS2 99/99 · flex 626/629 · position 92/105 · wm 781/781 · multicol **186/455** (+7) · spanner-frag 12/13.

**14b — Nested multicol leaf-frag.** Leaf block incorrectly spans inner sub-cols 1+2 instead of only sub-col 1 within the same outer column pass. Break token threading verified correct (Phase 12c); the `ClipBlockAxisOnly` fix reduced `multicol-nested-010` from 4500→3500px (F2.1). Remaining issue is in how `multicol_layout.go` or `block_layout.go`'s leaf path terminates the inner column pass at the outer fragmentainer boundary. Requires Blink source research (`column_layout_algorithm.cc:1080–1130`, `block_layout_algorithm.cc:2440–2500`) before coding. See `findings.md` §"Phase 14b research".

**14c — clear-001 clearance (deferred).** 96/480000 pixels (0.0%). Two unresolved hypotheses: (A) float paint uses `ToEnclosingRect`-style rect expansion, (B) float layout position differs. Root cause must be confirmed via debug logging before any code change. See `findings.md` §"Phase 14c research".

Files: `findings.md` (+~140 — three Phase 14{a,b,c} research subsections), `task_plan.md` (Phase 14 section + sub-phase table + discipline rules), `progress.md` (this section), `CONTINUE-14a.md` (new continuation prompt for Phase 14a implementation).
