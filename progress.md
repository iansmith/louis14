...
## Current Phase
**Phase 9 G-SINGLETONS — IN PROGRESS (7/11 closed).** Two landings:
...
css-position now **92/100 runnable**. Gates hold: wm 781/781, CSS2 99/99, flex 626/629.

**Remaining Phase 9 runnable:**
- `stack-floats-001.xht` (1.7%) — **paint-phase refactor pending.** See findings.md "Stack-floats-001 paint-phase analysis". Requires Blink-style `PaintPhaseBlockBackground` / `PaintPhaseFloat` / `PaintPhaseForeground` in `pkg/render`. Single-pass paint order cannot produce the spec's step-3 → step-4 → step-5 interleaving for block-in-inline + float + inline text. ~200-400 LOC; regression risk across all 6720 CSS3 tests.
...
- `position-absolute-iframe-print-001/002.sub.html` — **deferred out-of-scope.** Needs WPT server + cross-origin iframe loader. Harness infra.
...
| Question | Answer |
|----------|--------|
| Where am I? | css-position category, **92/100 runnable PASS**. Phase 1 (G-TABLE-REL) DONE; Phase 2 dissolved; Phase 3 (G-DYN-STATIC) DONE; Phase 4 (G-ABS-CENTER + G-HYPO, IMCB) DONE. **Phase 5: G-FIXED Part A + G-ROOT-FLEX-GRID DONE (M5a, M5b).** **Phase 6: G-ABS-IN-INLINE DONE (M6).** **Phase 7: G-STICKY DONE (commit `05aff97e`).** **Phase 8: G-REPLACED DONE (2026-04-21, commit `0e1fde9f`).** **Phase 9 G-SINGLETONS: 7/11 closed (first landing `a7e79598` 5 relpos, second landing `1bdcfc85` marker + `a22cfe10` button+flex-OOF).** Remaining: 1 paint-phase test (`stack-floats-001`, architectural decision pending), 3 deferred out-of-scope (`clear-001` Blink subpixel quirk, `iframe-print-001/002` harness infra), 1 parser bug (`position-change`). Phase 5 G-FIXED Part B (paint-clip/scrollTop) still pending; overlaps G-SCROLL. |
