...
**14a — IFC fragmentation guard + empty-child overflow trigger.** Root cause confirmed: `inline_layout.go:963` guard `blockOffset > 0` prevents IFC from breaking before its first line when `FragmentainerOffset > 0` (prior sibling already occupies part of the fragmentainer). Blink mirrors `refuse_break_before = (space_left >= fragmentainer_block_size)` — refuses only when the fragmentainer has received NO content. Fix: change to `bla.space.FragmentainerOffset + blockOffset > 0`. Part 2: when IFC produces height=0 + break token, `block_layout.go:895` doesn't detect overflow (blockCursor didn't advance). Add condition `childHasBreak && childBlockSize == 0 && fragSize != Indefinite && !IsInitialColumnBalancingPass`. Targets: 4 F4 regressions (`multicol-inherit-001`, `-margin-001`, `-margin-child-001`, `-nested-margin-001`). See `findings.md` §"Phase 14a research" for the full Blink reference.
