...
**Phases 13a + 13b + 13c + 13d + 13e (CLOSED) + 13e′ (CLOSED) + 13f (CLOSED) landed 2026-04-25.** Phase 13e closed at 13e.6 (entry-side migration via `ResolvePercent`); **Phase 13e′ closed at 13e′.3** (exit-side return-type promotion: 13e′.1 `ResolveInlineSize`+`ResolveBlockSize`, 13e′.2 `ResolveMinInlineSize`+`ResolveMaxInlineSize`, 13e′.3 `ResolveMinBlockSize`+`ResolveMaxBlockSize` — this commit closes); **Phase 13f closed at 13f.3** (13f.1 snap helpers + 13f.2 MeasureText* return-type promotion + 13f.3 ShapeAdvances*/Mixed return-type promotion to ShapeCumulative; 13f.4 closed as "skip"). With 13e′ closed, every public `Resolve*Size` API in `pkg/layout/fragment_geometry.go` returns `LayoutUnit` at the boundary, mirroring Blink's `length_utils.{h,cc}` exactly. ~90 consumer sites total bridged with `.Float64()` across the layout pipeline. **Rounding-mode discovery (during 13e′.1):** first attempt used `FromFloat64Trunc` to mirror Blink's `LayoutUnit(float)` ctor; regressed css-multicol 179 → 172 because louis14's float64 `Length.value` storage (vs Blink's float32) makes `8.2 * 50.0` land at IEEE 754 `409.99999...`, which Trunc snapped DOWN to 409.984 and shifted column-rule positions by 1 px. Fix: switch to `FromFloat64Round` — round-half-away-from-zero absorbs the IEEE 754 noise while preserving bit-exact round-trip for 1/64-clean inputs. The Round-mode pattern carried cleanly through 13e′.2 + 13e′.3 with no further rollbacks. All six gate invariants held at the 13e′.3 checkpoint (CSS2 99/99, flex 626/629, position **91/104**, wm 781/781, multicol 179/455, spanner-frag 12/13). **Queued: 13g (paint-time pixel snap, the most likely clear-001 closer), 13h (verification + cleanup, retro doc).** See `findings.md` "Phase 13e′ research" for the Blink-parity reference and the rounding-mode-discovery rollback notes, and `progress.md` "Phase 13e′.{1,2,3}" for landing details.

**Phase 12 (css-multicol)** remains the active layout-feature track at 179/455. Most active follow-up targets done: F1 (wm 781/781), F5 (multicol +3 list-item tests), HTML tokenizer EOF-recovery (css-position 91/104). F2/F3/F4 remain PARTIAL with documented residuals. **Phase 13 (LayoutUnit precision discipline)** is in active execution: 13a + 13b + 13c + 13d + 13e (all six sub-steps 13e.1–13e.6) **+ 13e′ (CLOSED: 13e′.1 + 13e′.2 + 13e′.3) + 13f (CLOSED: 13f.1 + 13f.2 + 13f.3, 13f.4 skipped)** all done; 13g + 13h queued.
