...
**Active task:** Phase 16.e — port Blink's spanner-layout subsystem. Two Blink-parity routes (both needed eventually; sequencing recommendation: option 2 first because it's more contained, then option 1 later):
1. **Finish the FinishFragmentation port.** The `len(children) == 0` gate added in 16.d.1 is louis14 scaffolding, not Blink. Blink's `FinishFragmentation` runs on every block; the parent-side children-loop overflow path in `block_layout.go:1001-1196` is louis14 divergence that conflicts with self-fragmentation. Delete or shrink the parent-side path; let `FinishFragmentation` handle non-leaves the same way it handles leaves.
2. **Port `LayoutSpanner` + spanner break-token shape.** Louis14's `layoutSpannerInFrag` + `pendingContentOverflow` / `spannerContentBreakToken` machinery in `multicol_layout.go:663-676 / 724` is a partial port of Blink's `LayoutSpanner` / `CreateConstraintSpaceForSpanner` / `BreakBeforeChildIfNeeded`. The break-token chain shape doesn't match Blink (Blink uses a `MulticolBreakTokenData` carrier per the Phase 18 brief). Port the exact Blink mechanism so the spanner-resume mechanism handles every pattern, not just the few patterns louis14's hybrid happens to cover.

**Then queue:** Phase 16.c.2 retry (third time, after option 2) → Phase 16.d.2/3 if needed → Phase 17.
