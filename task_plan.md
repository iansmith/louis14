...
**Phases 13a + 13b + 13c + 13d + 13e (CLOSED) + 13f.1 landed 2026-04-25.** Phase 13e closed at 13e.6 with every percentage-resolution call site routed through `layoutunit.ResolvePercent`. **Phase 13f (text-shaping boundary) is now in flight: 13f.1 landed.** 13f.1 added the additive snap helpers — `pkg/text/shape_snap.go` (`shapeWidthSnap` = CEIL, `shapeAdvancePairSnap` = FLOOR/CEIL pair) plus `TestShapeWidthSnap`/`TestShapeAdvancePairSnap` and `TestFloorCeilPairInvariant` covering the load-bearing 13f invariant (`End[i] - Start[i] ∈ {0, 1 raw quantum}`; ≤ 2-quanta total over arbitrarily many cluster offsets). All six gate invariants held at every of the 17 checkpoint commits today (CSS2 99/99, flex 626/629, position **91/104**, wm 781/781, multicol 179/455, spanner-frag 12/13). 13e′ (Resolve*Size return-type promotion, ~50-site ripple) + 13f.2/13f.3/13f.4 + 13g + 13h queued. See `findings.md` "Phase 13f research: text-shaping boundary snap discipline" for the Blink-parity reference (`shape_result.h` `SnappedWidth`/`SnappedStart/EndPositionForOffset`), and `progress.md` "Phase 13a/13b/13c/13d/13e.x/13f.1" for landing details.

**Phase 12 (css-multicol)** remains the active layout-feature track at 179/455. Most active follow-up targets done: F1 (wm 781/781), F5 (multicol +3 list-item tests), HTML tokenizer EOF-recovery (css-position 91/104). F2/F3/F4 remain PARTIAL with documented residuals. **Phase 13 (LayoutUnit precision discipline)** is in active execution: 13a + 13b + 13c + 13d + 13e (all six sub-steps 13e.1–13e.6) DONE; **13f.1 (text-shaping snap helpers, additive) DONE**; 13e′ (Resolve*Size return-type promotion) + 13f.2/13f.3/13f.4 + 13g + 13h queued.
