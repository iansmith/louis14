...
**Phases 13a + 13b + 13c + 13d + 13e (CLOSED) + 13f (CLOSED) landed 2026-04-25.** Phase 13e closed at 13e.6; **Phase 13f closed at 13f.3** (13f.1 snap helpers + 13f.2 MeasureText* return-type promotion + 13f.3 ShapeAdvances*/Mixed return-type promotion to ShapeCumulative; 13f.4 closed as "skip"). 13f.3 promoted `ShapeAdvances` and `ShapeAdvancesMixed` from `([]float64, bool)` to `(text.ShapeCumulative, bool)` — the Blink-parity floor/ceil-snapped pair (`shape_result.h` `SnappedStart/EndPositionForOffset`). The internal float64 cumulative-position computation is preserved (renamed to private `shapeAdvancesFloat`/`shapeAdvancesMixedFloat`); the snap happens once at the public API boundary via `shapeAdvancePairSnap`. The single louis14 consumer at `pkg/layout/line_breaker.go:1574` migrated from `cum[rng[1]] - cum[rng[0]]` to `cum.SnappedWidth(rng[0], rng[1]).Float64()` — a one-line change because the existing pattern was already pair-of-positions shaped. All six gate invariants held at every of the 19 checkpoint commits today (CSS2 99/99, flex 626/629, position **91/104**, wm 781/781, multicol 179/455, spanner-frag 12/13). **Queued: 13e′ (Resolve*Size return-type promotion to LayoutUnit, ~50-site ripple), 13g (paint-time pixel snap), 13h (verification + cleanup).** See `findings.md` "Phase 13f research: text-shaping boundary snap discipline" for the Blink-parity reference, and `progress.md` "Phase 13f.x" for landing details.

**Phase 12 (css-multicol)** remains the active layout-feature track at 179/455. Most active follow-up targets done: F1 (wm 781/781), F5 (multicol +3 list-item tests), HTML tokenizer EOF-recovery (css-position 91/104). F2/F3/F4 remain PARTIAL with documented residuals. **Phase 13 (LayoutUnit precision discipline)** is in active execution: 13a + 13b + 13c + 13d + 13e (all six sub-steps 13e.1–13e.6) **+ 13f (CLOSED: 13f.1 + 13f.2 + 13f.3, 13f.4 skipped)** all DONE; 13e′ (Resolve*Size return-type promotion) + 13g + 13h queued.
