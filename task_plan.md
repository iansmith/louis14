...
|---|---|---|---|---|---|
| F1 | wm `bidi-embed-006` + `bidi-override-006` (0.3% / 1598 px each) | css-writing-modes *Blink-parity unified shape pass* (NOT a regression — never passed) | +2 wm (toward 781/781) | **ACTIVE 2026-04-24 — re-diagnosed.** Box-tree dump showed wrapper geometry is identical between `.test` and `.ref` (both 62.8 px tall) — the prior "11 px shorter" diagnosis was wrong. Real cause: `canMergeShapingContext` refuses to merge text items across bidi-level boundaries (`line_breaker.go:1516, 1525`), so multi-fragment `.test` text becomes 5 independent HarfBuzz shape calls vs `.ref`'s single call → cross-call kerning lost → ~0.1 px drift per fragment boundary → glyph anti-aliasing differs → 1598 px diff. Mirror-glyph already works (HarfBuzz auto-detect → RTL → `rtlm`). **Plan (Option A, Blink-parity unified shape pass):** F1a add `ShapeAdvancesMixed` API in `pkg/text`; F1b wire into line_breaker; F1c paint-side consistency check. See findings §F1. | `bidi-embed-006.html`, `bidi-override-006.html` |
