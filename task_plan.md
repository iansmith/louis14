...
**Phases 13a + 13b + 13c + 13d + 13e (CLOSED) + 13e′ (CLOSED) + 13f (CLOSED) + 13g.1 (helper additive) landed 2026-04-25; 13g.2 (border-edge inner-corner migration) landed 2026-04-26.** Phase 13e closed at 13e.6 (entry-side migration via `ResolvePercent`); **Phase 13e′ closed at 13e′.3** (exit-side return-type promotion: 13e′.1 `ResolveInlineSize`+`ResolveBlockSize`, 13e′.2 `ResolveMinInlineSize`+`ResolveMaxInlineSize`, 13e′.3 `ResolveMinBlockSize`+`ResolveMaxBlockSize`); **Phase 13f closed at 13f.3** (13f.1 snap helpers + 13f.2 MeasureText* return-type promotion + 13f.3 ShapeAdvances*/Mixed return-type promotion to ShapeCumulative; 13f.4 closed as "skip"). **Phase 13g.1 (commit `776ae6d5`) added the additive `SnapSizeToPixel` + `SnapSizeToPixelAllowingZero` helpers to `pkg/geometry/layoutunit`** mirroring `third_party/blink/renderer/platform/geometry/layout_unit.h` verbatim, including the >4-raw thin-line clause. **Phase 13g.2 (commit `050cf822`) migrated the `drawBorders` inner-corner snap at `pkg/render/render.go:2672-2675`** — 4 ad-hoc `math.Round(x + bw.Left)` lines replaced with `outerEdge ± SnapSizeToPixel(bw, unsnapped_origin)`. Side-by-side instrumentation across 7 WPT border tests confirmed: thin borders (0.1, 0.25, 0.5px) now render correctly (old: thickness 0; new: thickness 1); normal borders (≥0.7px) unchanged. The migration also fixed a separate latent asymmetric-0.5px-border bug (left/top=1px, right/bottom=0px due to round-half-away-from-zero of `Round(792-0.5)` going UP). Parallel-sites scope correction: only `:2669` `drawBorders` had the inner-corner formula; `:2779` `drawRoundedBorders` (midline radius), `:2940` `drawOutline` (expand outward), `:4209` `drawOutsetBoxShadows` (shadow rect) are not 13g.2 targets. **Rounding-mode discovery (during 13e′.1):** first attempt used `FromFloat64Trunc` to mirror Blink's `LayoutUnit(float)` ctor; regressed css-multicol 179 → 172 because louis14's float64 `Length.value` storage (vs Blink's float32) makes `8.2 * 50.0` land at IEEE 754 `409.99999...`, which Trunc snapped DOWN to 409.984 and shifted column-rule positions by 1 px. Fix: switch to `FromFloat64Round` — round-half-away-from-zero absorbs the IEEE 754 noise while preserving bit-exact round-trip for 1/64-clean inputs. **Six-invariant gate sweep at 13g.2 HEAD (atop mazzy `d0105bd`): CSS2 99/99 · flex 626/629 · position 92/105 · wm 781/781 · multicol 179/455 · spanner-frag 12/13.** All numbers identical to the pre-13g.2 baseline — the migration is bit-exact for every WPT test in the gate sweep (the thin-border cases that change behavior are all `rel="mismatch"` tests skipped by the runner). clear-001 still PARTIAL (no borders → diff is from float/clear sub-pixel positioning, not border-edge snap). **Queued: 13g.3 (route `pixelSnap(x,y,w,h)` through `SnapSizeToPixel` — propagates thin-line preservation to all 13 callers, the next likely clear-001 closer), 13g.4+ (background/clip clusters one at a time), 13h (verification + cleanup, retro doc).** See `findings.md` "Phase 13e′ research" for the Blink-parity reference and the rounding-mode-discovery rollback notes, "Phase 13g research" for the SnapSizeToPixel port + ad-hoc Round inventory + 13g.2 verification subsection, and `progress.md` "Phase 13g.{1,2}" + "Detour: mazarin/textshape font-slot lifetime fix" for landing details.

**Phase 12 (css-multicol)** remains the active layout-feature track at 179/455. Most active follow-up targets done: F1 (wm 781/781), F5 (multicol +3 list-item tests), HTML tokenizer EOF-recovery (css-position 91/104). F2/F3/F4 remain PARTIAL with documented residuals. **Phase 13 (LayoutUnit precision discipline)** is in active execution: 13a + 13b + 13c + 13d + 13e (all six sub-steps 13e.1–13e.6) **+ 13e′ (CLOSED: 13e′.1 + 13e′.2 + 13e′.3) + 13f (CLOSED: 13f.1 + 13f.2 + 13f.3, 13f.4 skipped) + 13g.1 (additive helper) + 13g.2 (border-edge inner-corner migration)** all done; 13g.3/13g.4+ + 13h queued.
