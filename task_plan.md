...
| F3 | Phase 12f `column-height`/`column-wrap` cluster residuals (24 tests, 0.1–4.2 %) | css-multicol layout | +up to 24 multicol (partial closure likely) | **PARTIAL 2026-04-24.** Row-gap plumbing landed (`GetRowGapMulticol`, `mla.rowGapSize` set from style). `column-height-027` PASS; `-009` 20061→240 px near-pass; css-multicol 155→157 (+2). Biggest still-open: `-023` (10000 px), `-017` (7000 px), `-013` (6500 px) — all nested + spanner-row-gap + zero-column-height edge cases. Deferred, may share path with F2 phase 2. See findings §F3. | Done: `column-height-008.html` triaged; next driver `column-height-023.html` (biggest open) |
