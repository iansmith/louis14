...
## Current focus (2026-04-27 — Phase 16.c.2 retry ATTEMPTED + REVERTED again; queue → Phase 16.e (spanner-content fragmentation) → 16.c.2 retry → 16.d.2/3 → Phase 17)
...
**Phase 16.c.2 retry — ATTEMPTED + REVERTED 2026-04-27 (second attempt).** With Phase 16.d.1 in place, removing `ClipBlockAxisOnly` (setter + paint branch) gives multicol 192 → 195 (+3) with the IsInsideColumnSpanner gate kept, or 192 → 196 (+4) with that gate also removed. **Both variants regress 2-3 driver tests** that were passing at HEAD: `spanner-fragmentation-006` (0.2-0.3% diff — small red strip near top of inner col 2 + tiny green island below multicol) and `nested-floated-multicol-with-monolithic-child` (0.2% — green bleed below multicol box), and `spanner-fragmentation-004` (1.0% with gate kept, 0% with gate removed). Per CLAUDE.md "ALL tests must pass; 0.5% is failure just like 28%", net-positive doesn't justify these regressions. Reverted via `git checkout HEAD -- pkg/layout/multicol_layout.go pkg/render/paint_layer.go pkg/layout/block_layout.go`.

**Diagnosis.** The remaining 2-3 failing tests all have the same pattern: a column-spanner whose CHILDREN extend visually past the multicol container's box (e.g., `<div column-span:all; height:10><div height:360></div></div>`). With the clip in place, the 360h overflow inside the column gets clipped at column boundaries. Without the clip, the 360h overflow renders past the multicol — Blink would instead fragment that 360h into multiple OUTER columns (each holding a slice). Louis14's existing `pendingContentOverflow` + `spannerContentBreakToken` mechanism (`multicol_layout.go:663-676 / 724`) handles SOME spanner-overflow cases but produces an incorrect break-token chain when the leaf descendant self-fragments via Phase 16.d.1's clamp. The IsInsideColumnSpanner gate disables 16.d.1's clamp inside spanners, but then the spanner's overflow isn't fragmented at all — it just shows past the multicol.

**Active task:** DECISION POINT. Phase 16.e (port Blink's spanner-layout subsystem) was attempted with a "minimal unwrap" approach 2026-04-27 and reverted: the wrapper around `pendingPartialSpannerToken` turned out to be load-bearing in unexpected ways (defensive-copy boundary for the combined-clip mutation at `multicol_layout.go:762-770`; semantic distinction for `spannerConsumed` vs spanner content cursor). A proper port requires touching 6 sites in `multicol_layout.go` (positional read, pendingPartial build, clipToken append, nextSpannerClipToken chain, buildOuterBreakResult, BlockBreakToken shape) — Phase 18-equivalent refactor.

**Open question for next session:** continue Phase 16.e with full refactor (substantial), OR pivot to Phase 17 (Forced-break balance, ~5 tests, MEDIUM tractability per the brief) and come back to 16.e later? Per CLAUDE.md "Foundational correctness over quick wins" the refactor is right; per CLAUDE.md "Pick a category and solve it completely — don't skip around" we should finish multicol cleanup first. Either choice is defensible; the user should weigh "more multicol-gate gain via 17/19 first" vs "clean foundation via 16.e first".

**Why option 2 now (correctness rationale, important to future):** Louis14 currently has TWO competing spanner-content-overflow mechanisms — Blink's (partially ported in `pendingContentOverflow` / `spannerContentBreakToken`) and louis14's own positional encoding (`ChildBreakTokens[0]=nextCol, [1]=partialSpanner, [2]=colRows`). The latter is divergence from Blink's walker model (`MulticolPartWalker` dispatches break-tokens by `InputNode()`, flat list). The hybrid works for some patterns and fails for others (spanner-fragmentation-006, the 0.1% remainder after 16.d.1). The clip masks the failures. Per CLAUDE.md "If a fix doesn't generalize to all cases, it's the wrong fix" — keeping the hybrid + clip is the wrong fix. The right fix is to port Blink's walker model fully so spanner-content fragmentation works for every pattern, then drop the clip.

**Phase 16.e plan (option 2):**
- **16.e.1** Read Blink source carefully: `column_layout_algorithm.cc::LayoutSpanner`, `CreateConstraintSpaceForSpanner`, `BreakBeforeChildIfNeeded`, and `MulticolPartWalker::UpdateCurrent` (Blink uses break-token `InputNode()` to discriminate between multicol-container, spanner, and OOF on resume).
- **16.e.2** Port `MulticolPartWalker`-style resume to `multicol_layout.go`: replace positional `ChildBreakTokens[1]/[2]` with a walker that dispatches by `InputNode()`. Spanner break-tokens become regular `BlockBreakToken` with `Node = spanner` (no extra wrapping).
- **16.e.3** Drop the `IsInsideColumnSpanner` gate in `block_layout.go:1410-1415` (and the propagation at line 612-614). Spanner descendants now self-fragment normally; the walker handles their break-tokens correctly via `InputNode()` dispatch.
- **16.e.4** Diagnose + fix `nested-floated-multicol-with-monolithic-child` separately (no spanner involved; likely a small `contain:size` / float-overflow fix).
- **16.e.5** Verify all 13 driver tests + 5 originally-recovered tests + multicol gate.
- **16.c.2 retry #3** Delete clip + paint branch. Should now be net-positive cleanly.

**Then queue:** Phase 16.c.2 retry → Option 1 (FinishFragmentation full port — drop the leaf-only gate, delete parent-side overflow path) → Phase 17.
...
7. **Phase 16.e** (NEW, prerequisite for 16.c.2 retry) — Blink-parity spanner-content fragmentation. The 16.c.2 second-attempt retry showed that even after 16.d.1, removing the clip regresses 2-3 spanner-overflow tests. Need to either (a) extend 16.d.1 to handle spanner descendants properly + fix the spanner-resume mechanism in `multicol_layout.go:663-676 / 724` to consume the new break-token chain, or (b) implement Blink's specific spanner-content slicing. Brief to be written.
8. **Phase 16.c.2 retry** (third attempt, after 16.e) — Remove `ClipBlockAxisOnly` setter + paint-side branch. Should be safe once spanner content properly fragments.
9. **Phase 17** — Forced-break balance (T2, ~5 tests, MEDIUM). Rewrites `resolveColumnAutoBlockSize` (`multicol_layout.go:1396`) with Blink-parity `ContentRun`/`ContentRuns`/`DistributeImplicitBreaks` measure-pass loop. Brief: `findings.md` § Phase 17.
10. **Phase 18** — Nested multicol break-token forwarding (T3, ~15 tests, HARD). Adds `MulticolBreakTokenData` carrier on `BlockBreakToken`. Brief: `findings.md` § Phase 18.
11. **Phase 19** — span-all-children-height 002-013 (T4, 12 tests, MIXED). 7 sub-clusters. Brief: `findings.md` § Phase 19.

**Gate invariants (committed at HEAD `c40b4b56`):** CSS2 99/99 · flex 626/629 · css-position 92/105 · wm 781/781 · multicol **192/455** · spanner-fragmentation 11/13.

**Gate invariants (target after 16.c.2 retry):** CSS2 99/99 · flex 626/629 · css-position 92/105 · wm 781/781 · multicol **195+/455** · spanner-fragmentation 12+/13.
