...
| F4 | Phase 12h.2 inline-in-balanced-multicol | css-multicol layout | +2 multicol (`-large-001`, `-stacking-001`) + capability that also closes F2 phase 2 (`multicol-nested-010` + siblings, ~3-7 tests) | **RECOMMENDED NEXT 2026-04-24.** Research DONE (findings §F4): Blink entry chain `cla.cc:763` → `bla.cc:593` → `ila.cc:1071`, `InlineBreakToken.start_.item_index`/`start_.text_offset` resume semantics, and a concrete hypothesis matching our `RenderedColumnCount=1/2` symptom (missing `params.break_token = column_break_token` forwarding and/or missing `SetBreakToken(line_info.GetBreakToken())`). Mechanical fix with clear Blink algorithm to mirror. Higher expected impact than remaining F3 residuals. | `multicol-rule-stacking-001.xht` |
