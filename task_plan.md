...
**Phases 13a + 13b landed 2026-04-25.** Foundational LayoutUnit scalar (`pkg/geometry/layoutunit`) and composite geometry types + WM conversions (`pkg/geometry`). 27 unit tests across the two packages; all six gate invariants held by construction. No callers yet; 13c is the first sub-phase that migrates an existing `pkg/layout` `float64` field. See "Phase 13 sub-phases" table and `progress.md` "Phase 13a/13b" sections for landing details.
