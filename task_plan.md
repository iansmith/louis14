...
**Phases 13a + 13b + 13c + 13d + 13e (CLOSED) + 13f (CLOSED) landed 2026-04-25; Phase 13e′.1 landed 2026-04-25.** Phase 13e closed at 13e.6; **Phase 13f closed at 13f.3** (13f.1 snap helpers + 13f.2 MeasureText* return-type promotion + 13f.3 ShapeAdvances*/Mixed return-type promotion to ShapeCumulative; 13f.4 closed as "skip"). **13e′.1 (this commit) promoted `ResolveInlineSize` + `ResolveBlockSize` from `(float64, bool)` to `(layoutunit.LayoutUnit, bool)`**, mirroring Blink's `length_utils.{h,cc}` where every `Resolve*Length` returns LayoutUnit directly. ~40 consumer sites bridged with `.Float64()`. **Rounding-mode discovery during 13e′.1:** first attempt used `FromFloat64Trunc` to mirror Blink's `LayoutUnit(float)` ctor; regressed css-multicol 179 → 172 because louis14's float64 `Length.value` storage (vs Blink's float32) makes `8.2 * 50.0` land at IEEE 754 `409.99999...`, which Trunc snapped DOWN to 409.984 and shifted column-rule positions by 1 px. Fix: switch to `FromFloat64Round` — round-half-away-from-zero absorbs the IEEE 754 noise while preserving bit-exact round-trip for 1/64-clean inputs. All six gate invariants held at the 13e′.1 checkpoint (CSS2 99/99, flex 626/629, position **91/104**, wm 781/781, multicol 179/455, spanner-frag 12/13). **Queued: 13e′.2 (ResolveMin/MaxInlineSize → LayoutUnit, ~25 sites), 13e′.3 (ResolveMin/MaxBlockSize → LayoutUnit, ~25 sites), 13g (paint-time pixel snap), 13h (verification + cleanup).** See `findings.md` "Phase 13e′ research" for the Blink-parity reference and the rounding-mode-discovery rollback notes, and `progress.md` "Phase 13e′.1" for landing details.

**Phase 12 (css-multicol)** remains the active layout-feature track at 179/455. Most active follow-up targets done: F1 (wm 781/781), F5 (multicol +3 list-item tests), HTML tokenizer EOF-recovery (css-position 91/104). F2/F3/F4 remain PARTIAL with documented residuals. **Phase 13 (LayoutUnit precision discipline)** is in active execution: 13a + 13b + 13c + 13d + 13e (all six sub-steps 13e.1–13e.6) **+ 13f (CLOSED: 13f.1 + 13f.2 + 13f.3, 13f.4 skipped) + 13e′.1 (DONE)** all done; 13e′.2 + 13e′.3 + 13g + 13h queued.
