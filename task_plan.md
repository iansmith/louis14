...
**Phases 13a + 13b + 13c + 13d + 13e (CLOSED) + 13f.1 + 13f.2 landed 2026-04-25.** Phase 13e closed at 13e.6. **Phase 13f (text-shaping boundary) is in flight: 13f.1 + 13f.2 landed; 13f.4 closed as "skip".** 13f.2 promoted `pkg/text.MeasureText` and `MeasureTextVerticalFromFont` return types from `(float64, float64)` to `(layoutunit.LayoutUnit, layoutunit.LayoutUnit)`, snapping via Blink's `ShapeResult::SnappedWidth` CEIL discipline at the public API boundary. The legacy float64-returning wrappers (`MeasureTextDefault`/`WithWeight`/`WithStyle`/`Vertical`) keep their float64 signatures (only `louis13/` consumes them; ~46 louis13 sites stay untouched) and bridge their bodies via `.Float64()` so the snap discipline still applies internally. Active louis14 consumers (`pkg/layout/engine.go` 1 site + `pkg/layout/line_breaker.go` 32 sites) bridge with `.Float64()` at the call site. All six gate invariants held at every of the 18 checkpoint commits today (CSS2 99/99, flex 626/629, position **91/104**, wm 781/781, multicol 179/455, spanner-frag 12/13). 13e′ + 13f.3 + 13g + 13h queued. See `findings.md` "Phase 13f research: text-shaping boundary snap discipline" for the Blink-parity reference, and `progress.md` "Phase 13f.x" for landing details.

**Phase 12 (css-multicol)** remains the active layout-feature track at 179/455. Most active follow-up targets done: F1 (wm 781/781), F5 (multicol +3 list-item tests), HTML tokenizer EOF-recovery (css-position 91/104). F2/F3/F4 remain PARTIAL with documented residuals. **Phase 13 (LayoutUnit precision discipline)** is in active execution: 13a + 13b + 13c + 13d + 13e (all six sub-steps 13e.1–13e.6) + 13f.1 (snap helpers) + **13f.2 (MeasureText* return-type promotion to LayoutUnit) DONE**; 13f.4 closed as "skip"; 13e′ (Resolve*Size return-type promotion) + 13f.3 (ShapeAdvances* return-type promotion) + 13g + 13h queued.
...
| 13f | **Text-shaping boundary** — `pkg/text` exposes `MeasureText` returning `LayoutUnit` (via `FromFloat64Ceil` analog of `ShapeResult::SnappedWidth`). `ShapeAdvances`/`ShapeAdvancesMixed` return paired floor/ceil-snapped LayoutUnit cumulative-position slices (Blink's `SnappedStart/EndPositionForOffset`); consumers read pair-of-positions `End[e].Sub(Start[s])`. HarfBuzz output stays `float64` *inside* the `mazarin/textshape` package. **Sub-steps:** 13f.1 (additive snap helpers + tests) — DONE 2026-04-25 (`c5c9b67c`); **13f.2 (`MeasureText` + `MeasureTextVerticalFromFont` → LayoutUnit, 33 active louis14 consumer bridges; legacy `MeasureTextWithStyle`/`WithWeight`/`Default`/`Vertical` keep float64 sig with internal bridge so louis13's ~46 sites stay untouched) — DONE 2026-04-25**; 13f.3 (`ShapeAdvances*` → paired LayoutUnit, 1 consumer bridge at `line_breaker.go:1546` — already pair-of-positions shaped); 13f.4 (mazarin `TextRunLayoutUnit` coordination — CLOSED as "skip" per `findings.md` 13f research; HarfBuzz int32 26.6 → /64.0 → LayoutUnit raw is already lossless, no shaper-internal type swap needed). | `pkg/text/{shape_snap.go,measure.go}`, `pkg/layout/{line_breaker.go,engine.go}`. | Verify wm 781/781 holds; sub-pixel kerning behavior may shift slightly. | Medium. |
