...
**Active task:** Phase 16.e — port Blink's spanner-layout subsystem. **Decision: do option 2 now.** Per CLAUDE.md "Foundational correctness over quick wins, no easy wins, no near-passing tests". Both ports (1 and 2 below) are needed for full Blink parity; the choice is sequencing, not philosophy. Option 2 is more contained (touches `multicol_layout.go` + `BlockBreakToken` resume path) and unblocks 16.c.2 retry directly. Option 1 (delete the parent-side children-loop overflow path in `block_layout.go:1001-1196` and let `FinishFragmentation` handle non-leaves) is the bigger structural change and is queued as a separate phase after 16.c.2 lands.
