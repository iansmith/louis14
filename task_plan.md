...
**Active task:** Document the retry attempt and queue Phase 16.e — proper Blink-parity spanner-content fragmentation. Read Blink `column_layout_algorithm.cc::LayoutSpanner` more carefully (especially `BreakBeforeChildIfNeeded` for spanner children + how the break-token chain is structured for spanner-content resume). Key Blink behavior to mirror: the spanner box is monolithic for placement (breaks BEFORE as a unit), but its children CAN fragment across outer multicol columns — and the break-token chain must allow the spanner-resume mechanism to lay out the leaf at its progress point in subsequent outer columns.

**Then queue:** Phase 16.e (spanner-content fragmentation Blink-parity) → 16.c.2 retry (third time) → Phase 16.d.2/3 if needed → Phase 17.
