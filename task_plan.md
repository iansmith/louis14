...
**Phase 12 (css-multicol)** remains the active layout-feature track at **186/455** (post-14a). Most active follow-up targets done: F1 (wm 781/781), F5 (multicol +3 list-item tests), HTML tokenizer EOF-recovery (css-position 91/104). F2/F3/F4 remain PARTIAL with documented residuals. **Phase 13 (LayoutUnit precision discipline) CLOSED 2026-04-26:** 13a + 13b + 13c + 13d + 13e (13e.1–13e.6) + 13e′ (13e′.1–13e′.3) + 13f (13f.1–13f.3, 13f.4 skip) + 13g.1–13g.4 + 13h all DONE. Follow-ups: clear-001 (float-paint `ToEnclosingRect` discipline), `drawColumnRules` content-area origin (render.go:2931-2933), consumer-accumulator promotion sweep.

**Detour 2026-04-26: mazarin/textshape font-slot lifetime fix (mazzy `d0105bd`).** While prepping the 13g.2 gate sweep, discovered that `f41f5c4d` (render-side OpenTemporaryFont landing, "unrelated" per the original 13g brief) had introduced a SIGSEGV in `mazarin/textshape.parseSTypoMetrics` after ~89 fonts opened in any WPT section, plus 13 silent pixel-diff regressions in CSS2 from stale fontIDs in `pkg/text.fontIDCache`. Root cause: `CloseTemporaryFont` nilled permanent slots and Munmap'd shared mmap regions. Fix in mazarin/textshape adds an `isTemporary` slot bit + base-address-shared check in `CloseTemporaryFont`. **All six gate invariants verified at HEAD + the mazarin fix: CSS2 99/99 · flex 626/629 · position 92/105 · wm 781/781 · multicol 179/455 · spanner-frag 12/13.** No louis14 source change needed (louis14 picks up via `replace mazarin/textshape => ../mazzy/...` in go.mod). See `progress.md` "Detour: mazarin/textshape font-slot lifetime fix" for the full root-cause walk and gate-sweep numbers.
...
| 13g | **Paint-time pixel snap** — port `SnapSizeToPixel(size, location)` (and `SnapSizeToPixelAllowingZero` companion) to `pkg/geometry/layoutunit` (mirrors Blink's `platform/geometry/layout_unit.h` placement, NOT `pkg/render` per original brief — Blink keeps the primitive in geometry; painters call via rect-wrappers like `PhysicalRect::PixelSnappedWidth/Height`). Replaces ad-hoc `math.Round` in border/background-edge painting. The "preserve thin lines" rule (Blink's `>4 raw → ±1 px` clause for lines below 1/16 px that would round to 0) closes the class of "0.5px border vanishes" bugs. **13g.1 DONE 2026-04-25** — additive helper in `pkg/geometry/layoutunit/layoutunit.go`; 24 unit tests; no callers. **13g.2 DONE 2026-04-26** — migrated `drawBorders` inner-corner snap at `pkg/render/render.go:2672-2675` (the only site with the inner-corner ad-hoc `math.Round` shape; parallel sites at :2779 `drawRoundedBorders`, :2940 `drawOutline`, :4209 `drawOutsetBoxShadows` use different geometry — radius midline / outline expand / shadow rect — and are not 13g.2 targets). Side-by-side instrumentation across 7 WPT border tests confirmed: thin borders (0.1, 0.25, 0.5px) now render correctly (old: thickness=0 collapsed, new: thickness=1 preserved); normal borders (≥0.7px) unchanged. Also fixes a separate latent bug: 0.5px borders previously had asymmetric thickness (left/top=1px, right/bottom=0px) due to round-half-away-from-zero of `Round(792-0.5)` going up to 792. All six gate invariants held at 13g.2 HEAD. **13g.3 DONE 2026-04-26** — migrated `pixelSnap(x,y,w,h)` at render.go:1525-1531 to compute snap size via `SnapSizeToPixel(luW, luX)` / `SnapSizeToPixel(luH, luY)` (origin still `math.Round`, mirrors Blink `ToPixelSnappedRect` at `physical_rect.h:224`). All 13 callers inherit the thin-line clause + 1/64-quantum determinism. Side-by-side instrumentation: integer-size cases bit-exact OLD vs NEW (clear-001, all gate-sweep tests); a 0.5×0.5px box at 0.5px origin transitions OLD w=0/h=0 (vanish) → NEW w=1/h=1 (preserved). **clear-001 hypothesis falsified — instrumentation showed bit-exact OLD/NEW for every clear-001 paint call**; clear-001 likely needs a separate float-paint discipline (Blink may use `ToEnclosingRect` floor-offset/ceil-far-edge for floats). All six gate invariants held at 13g.3 HEAD. **13g.4 DONE 2026-04-26** — migrated background-origin switch block at render.go:1875-1889 (12 `math.Round` edge-difference lines → 3 `pixelSnap` calls, one per case: border-box / content-box / padding-box). Blink research (via `BackgroundImageGeometry::AdjustPositioningArea()`) confirmed `ToPixelSnappedRect` discipline for all three cases — exact match to louis14's `pixelSnap`. Instrumentation skipped (Blink-parity confirmed, bit-exact for integer-px layouts). All six gate invariants held at 13g.4 HEAD: CSS2 99/99 · flex 626/629 · position 92/105 · wm 781/781 · multicol 179/455 · spanner-frag 12/13. | `pkg/geometry/layoutunit/{layoutunit,layoutunit_test}.go` for 13g.1; `pkg/render/render.go` for 13g.2 (4 lines replaced) and 13g.3 (5 lines replaced with 8 LayoutUnit + SnapSizeToPixel lines + new doc-comment citing physical_rect.h). | clear-001 PARTIAL — bit-exact for 13g.3 (instrumentation confirmed); needs separate float-paint enclosing-rect work. Thin-border / thin-box `rel="mismatch"` WPT tests skipped by the runner; verified via instrumentation that they now render. | Medium. |
| 13h | **Verification + cleanup + retrospective — DONE 2026-04-26.** Gate sweep confirmed 13g.4 baseline held at all six invariants (CSS2 99/99 · flex 626/629 · position 92/105 · wm 781/781 · multicol 179/455 · spanner-frag 12/13). math.Round audit clean — all remaining sites classified; one new follow-up (`drawColumnRules` content-area at render.go:2931-2933, paint-edge positioning, not migrated). Retrospective in findings.md §"Phase 13g retrospective": arc summary, thin-line clause, open follow-ups, architectural lesson. | `findings.md`, `task_plan.md`, `progress.md`. | Phase 13 CLOSED. | Low → delivered. |
...
|---|---|---|---|---|
| 14a | IFC fragmentation guard + empty-child overflow trigger | `pkg/layout/inline_layout.go:963`, `pkg/layout/block_layout.go` | **DONE** (`87d06be5`) — closed 4 F4 regressions; multicol 179→186 (+7) | Medium — hot path |
...
5. [x] **Deferred items landed as Phase 12h.6 (2026-04-26).** Three commits: Track B `af2bbb77`, Track C `b66e7dba`, Track A `058a5442`. Gate: CSS2 99/99 · flex 626/629 · pos 92/105 · wm 781/781 · multicol 188/455 · spanner-frag 12/13.

### Phase 12h.6 — Blink-parity port of deferred abstractions — **DONE 2026-04-26**
