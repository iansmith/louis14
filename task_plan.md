...
- [ ] **Remaining Phase 9 runnable tests (triaged 2026-04-21):**
  - `stack-floats-001.xht` (1.7%): CSS 2.1 Appendix E stacking of floats + inlines + block-in-inline. **Needs paint-phase refactor** — see findings.md "Stack-floats-001 paint-phase analysis". Single-pass paint order cannot separate step 3 (block bgs) from step 5 (inline content) across step 4 (floats). Blink uses `PaintPhaseBlockBackground` / `PaintPhaseFloat` / `PaintPhaseForeground`. Scope: ~200-400 LOC in `render.go` + `paint_layer.go`; regression risk across all 6720 CSS3 tests (any float+inline scenario). Deferred pending architectural decision.
  - `clear-001.xht` (96px): **Out of scope (Blink quirk test).** Ref hardcodes blue=97px/orange=95px for `height:1in` divs (CSS inch = 96px). Blink's 1in rendering produces 97+95 via sub-pixel rounding; our engine renders exact 96+96 per CSS spec (1in = 96px). Matching Blink's quirk would require introducing Blink's `LayoutUnit` fixed-point arithmetic + the specific sub-pixel rounding path. Not a layout/paint bug. Defer until a sub-pixel-rounding category is scheduled.
  - `position-absolute-iframe-print-001/002.sub.html` (1491px each): **Out of scope (harness infra).** Uses `{{hosts[alt][www]}}:{{ports[http][0]}}` WPT server substitutions to load cross-origin iframe content. Our harness has no WPT server + no cross-origin iframe loader. Not a layout bug. Defer until iframe content-loading infrastructure is built.
