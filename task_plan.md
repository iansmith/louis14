...
|---|---|---|---|---|---|
| F1 | wm `bidi-embed-006` + `bidi-override-006` (0.3% / 1598 px each) | css-writing-modes *@font-face layout-time provider registration* + *Blink-parity unified shape pass* (NOT a regression — never passed) | +2 wm (toward 781/781) | **ACTIVE 2026-04-24 — re-diagnosed twice.** First re-diagnosis (sub-pixel kerning from per-fragment shaping) was also wrong; debug-traced layout-time `openFont` and found it returns -1 for the @font-face Hebrew font `ezra_silregular`, so `measureWidth` falls back to `len(text) × fontSize × 0.6`. Multi-fragment `.test` line 2 sums to 245 (5 items, fits 1 line via overflow tolerance) while `.ref`'s single 18-byte item word-wraps to 202+29 across 2 lines — different wrap geometry drives the 1598 px diff. This is exactly the deferred Phase 12h Step 1 limitation: "@font-face families with names not in fonts.csv... real fix is `DirectGlyphProvider.RegisterFile`... Deferred". F1 is the visible test that demands it. **Plan:** F1d (the actual closer) plumb `DirectGlyphProvider.RegisterFile` and call from both renderer (post-`SetFonts`) AND layout engine pre-`Layout()`; F1a/F1b (already-implemented Blink-parity unified shape infra: `ShapeAdvancesMixed` + relaxed `canMergeShapingContext`) currently no-op for F1 but useful once F1d makes shaping actually fire — sub-pixel cross-fragment kerning may still need it. Mirror-glyph already works via HarfBuzz auto-detect. See findings §F1. | `bidi-embed-006.html`, `bidi-override-006.html` |
