...
|---|--------|-------|-------------|
| **B1** | Counter scope/nesting tree (`counter-001..004`, `counters-001..006`, `counters-scope-001..004`, `counter-slot-order*`, `counter-invalid`, `counter-important`-adjacent) | ~19 | Flat `map[string][]int`; no scope push/pop, no stale-sibling removal, no `counters()` join |
...
| **B5** | `@counter-style` / `CounterStyle` + predefined `list-style-type` algorithms (`list-style-type-decimal-line-height`, decimal/alphabetic correctness across markers, `calc-in-counter`-adjacent) | ~4 + cross-cutting | No `CounterStyle` object, no range/pad/negative/extends |
