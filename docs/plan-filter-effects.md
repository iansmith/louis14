...
- ~~`svg-empty-hidden-foreignobject-with-filter-001` (2.1%): no `<foreignObject>` support + filter painters bail on empty userBBox~~ — **CLOSED** by [LOU-134](https://linear.app/mazarin/issue/LOU-134/). Full `<foreignObject>` HTML-embedding (Part A) wasn't actually needed — the failing test uses a hidden/empty foreignObject as a wrapper for a filter attribute. Two minimal fixes: (1) run `trySVGContainerFilterDispatch` BEFORE the `svgIsVisible` early-return so visibility:hidden + flood-filter still produces the filter output (per CSS Filter Effects 1 §7); (2) drop the empty-userBBox bail in `paintSVGContainerWithFilter` so a flood-only filter with an explicit userSpaceOnUse region renders independent of source-graphic extent. Test PASS @ 0% diff.
- ~~`svg-multiple-filter-functions` (2.1%): mixed `url(...)+CSS-shorthand` filter list — `filter_effect_builder.go` explicitly skips `url()` in mixed lists (deferred FilterChain merge)~~ — **CLOSED** by [LOU-135](https://linear.app/mazarin/issue/LOU-135/). `BuildFilterEffect` now chains url() entries: each sub-filter's `LastEffect` feeds the next stage's SourceGraphic via new `SVGFilterBuilder.SetSourceOverride` (Blink's FilterChain composition). SVG shape painter gained a chain entry point. Test PASS @ 0% diff; no regression on single-url `url(#missing)` / cycle-fallback paths.
- ~~`blur-text` (13.2%): inline elements with `filter:` don't promote to paint layer; `BuildReferenceFilter` never invoked~~ — **CLOSED** by [LOU-136](https://linear.app/mazarin/issue/LOU-136/). Hypothesis was wrong: inline-layer promotion already worked (reference using `filter: blur(4px)` rendered correctly). The actual bugs were upstream parsing: (1) `ParseInlineStyle` naively split on `;`, mangling `filter: url("data:image/svg+xml;utf8,...")` at the inner `;`; (2) `readAttributeValue` didn't decode HTML character references, so `&quot;` survived. Fixed by routing `ParseInlineStyle` through the existing `splitDeclarationParts` (paren+quote+escape aware) and calling `gohtml.UnescapeString` on attribute return paths. Test PASS @ 0% diff.
- `svg-relative-urls-001` (2.1% post-Phase-2; was 4.3%): gate test closes at 0% under [LOU-137](https://linear.app/mazarin/issue/LOU-137/) v1 (master `c267e56d`), **but that fix is a louis14-specific URL string-bake/un-bake hack, not a Blink mirror.** The Blink-citation audit (master `2c33305e`) reopened LOU-137 for a foundational refactor: parse-time absolute URL resolution on `html.Document.BaseURL`, replacing the pre-bake shortcut in `ResolveRelativeURLsInHTML` and deleting the v1 `Node.IframeBase` / `AdoptNodeFromIframe` workaround. v2 must keep the gate at 0% while removing the hack.
