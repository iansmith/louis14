...
// ============================================================================
// Inline Layout Abstraction (supports both single-pass and multi-pass)
// ============================================================================

// InlineLayoutAlgorithm determines which algorithm to use for inline layout
type InlineLayoutAlgorithm int

const (
	// SinglePassAlgorithm uses the original single-pass inline layout algorithm
	SinglePassAlgorithm InlineLayoutAlgorithm = iota
	// MultiPassAlgorithm uses the multi-pass (Blink-style) inline layout algorithm
	MultiPassAlgorithm
)

// hasPseudoElements checks if a node has ::before or ::after pseudo-element content.
// This is used to determine if multi-pass layout can be safely used (it can't handle
// pseudo-elements yet).
...
// InlineLayoutResult contains the results of inline layout
...
// layoutInlineChildren handles the complete inline layout process including
// ::before pseudo-elements, regular children, and ::after pseudo-elements.
//
// This function is extracted from layoutNode to enable:
// 1. Independent testing of inline layout logic
// 2. Support for both single-pass (current) and multi-pass (new) modes
// 3. Cleaner separation of concerns
//
// Parameters:
// - node: The container node whose children are being laid out
// - box: The container box (already created by layoutNode)
// - display: The display type of the container
// - style: The container's computed style
// - border, padding: Box model values for the container
// - x: The container's X position
// - childY: Starting Y position for children
// - childAvailableWidth: Available width for child layout
// - contentWidth: Content width for text-align
// - isObjectImage: Whether this is an object element with loaded image (skip children)
// - computedStyles: Style map for all nodes
...
// - algorithm: Which layout algorithm to use (SinglePass or MultiPass)
//
// Returns: InlineLayoutResult with child boxes and final inline context
func (le *LayoutEngine) layoutInlineChildren(
	...
	if effectiveAlgorithm == SinglePassAlgorithm {
